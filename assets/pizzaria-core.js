...
(function(root){
'use strict';
const ITEMS={massa:{name:'Massa',pack:5,cost:5},molho:{name:'Molho',pack:5,cost:5},queijo:{name:'Muçarela',pack:5,cost:10},tomate:{name:'Tomate',pack:5,cost:5},calabresa:{name:'Calabresa',pack:5,cost:5},cogumelo:{name:'Cogumelo',pack:5,cost:5},manjericao:{name:'Manjericão',pack:5,cost:5},presunto:{name:'Presunto',pack:5,cost:5},ovo:{name:'Ovo',pack:5,cost:5},cebola:{name:'Cebola',pack:5,cost:5},azeitona:{name:'Azeitona',pack:5,cost:5},pimentao:{name:'Pimentão',pack:5,cost:5},milho:{name:'Milho',pack:5,cost:5},frango:{name:'Frango',pack:5,cost:5},requeijao:{name:'Requeijão',pack:5,cost:10},bacon:{name:'Bacon',pack:5,cost:5},oregano:{name:'Orégano',pack:5,cost:5},parmesao:{name:'Parmesão',pack:5,cost:10},gorgonzola:{name:'Gorgonzola',pack:5,cost:10}};
const RECIPES=[{id:'queijo',name:'Muçarela',toppings:[]},{id:'marguerita',name:'Marguerita',toppings:['tomate','manjericao']},{id:'calabresa',name:'Calabresa',toppings:['calabresa']},{id:'bosque',name:'Do bosque',toppings:['cogumelo','manjericao']},{id:'horta',name:'Da horta',toppings:['tomate','cogumelo']},{id:'especial',name:'Especial',toppings:['calabresa','tomate','manjericao']},
 {id:'portuguesa',name:'Portuguesa',toppings:['presunto','ovo','cebola','azeitona','pimentao','oregano']},
 {id:'frango-cremoso',name:'Frango com requeijão',base:'requeijao',toppings:['frango','milho','oregano']},
 {id:'bacon-milho',name:'Bacon com milho',toppings:['bacon','milho']},
 {id:'quatro-queijos',name:'Quatro queijos',toppings:['requeijao','parmesao','gorgonzola']},
 {id:'calabresa-cebola',name:'Calabresa acebolada',toppings:['calabresa','cebola','azeitona','oregano']},
 {id:'vegetariana',name:'Vegetariana',toppings:['tomate','cogumelo','milho','pimentao','azeitona']},
 {id:'presunto',name:'Presunto e queijo',toppings:['presunto','tomate','oregano']},
 {id:'bosque-cremoso',name:'Cogumelos com requeijão',base:'requeijao',toppings:['cogumelo','cebola','manjericao']},
 {id:'frango-bacon',name:'Frango com bacon',toppings:['frango','bacon','milho']},
 {id:'portuguesa-suave',name:'Portuguesa sem cebola',toppings:['presunto','ovo','azeitona','pimentao','oregano']}];
const base=a=>recipe(a.recipe).base||'queijo';
const brushTool=(a,key)=>key==='molho'||key===base(a);
...
function fresh(){return {version:2,cash:30,stock:Object.fromEntries(Object.keys(ITEMS).map(k=>[k,['massa','molho','queijo'].includes(k)?5:3])),served:0,revenue:0,tips:0,best:0,active:null,last:null,recent:[],clientOrders:{}};}
function normalize(raw){const p=fresh();if(!raw||typeof raw!=='object')return p;for(const k of ['cash','served','revenue','tips','best'])p[k]=integer(raw[k],p[k],k==='best'?100:999999);for(const k of Object.keys(ITEMS))p.stock[k]=integer(raw.stock?.[k],p.stock[k],999);
 p.recent=Array.isArray(raw.recent)?raw.recent.filter(id=>RECIPES.some(r=>r.id===id)).slice(-3):[];
 for(let id=0;id<4;id++)if(RECIPES.some(r=>r.id===raw.clientOrders?.[id]))p.clientOrders[id]=raw.clientOrders[id];
 ...
 n.toppings=Array.isArray(a.toppings)?a.toppings.filter(t=>t&&Object.hasOwn(ITEMS,t.kind)&&!['massa','molho','queijo'].includes(t.kind)).slice(0,144).map(t=>({kind:t.kind,x:coord(t.x),y:coord(t.y)})):[];
...
function newOrder(p,rng=Math.random){
 if(p.active)return p.active;
 const pick=list=>list[Math.min(list.length-1,Math.max(0,Math.floor((Number(rng())||0)*list.length)))];
 const client=p.served===0?0:pick([0,1,2,3].filter(id=>id!==p.last?.client));
 const recent=p.recent||[],previous=p.clientOrders?.[client];
 const choices=RECIPES.filter(r=>!recent.includes(r.id)&&r.id!==p.last?.recipe&&r.id!==previous);
 const r=p.served===0?RECIPES[0]:pick(choices.length?choices:RECIPES.filter(r=>r.id!==p.last?.recipe));
 p.active={id:p.served+1,recipe:r.id,client,slices:p.served===0?6:pick([4,6,8]),stage:'order',bakeMs:0,used:{},sauce:[],cheese:[],toppings:[],cuts:[]};return p.active;
}
function needs(a){return ['massa','molho',base(a),...recipe(a.recipe).toppings];}
...
function brush(p,key,point){const a=p.active;if(!a||a.stage!=='prep'||!brushTool(a,key)||!inside(point))return false;const list=key==='molho'?a.sauce:a.cheese;const cells=CELLS.flatMap(([x,y],i)=>Math.hypot(point.x-x,point.y-y)<.125&&!list.includes(i)?[i]:[]);if(!cells.length||!use(p,key))return false;list.push(...cells);return true;}
function topping(p,key,point){const a=p.active;if(!a||a.stage!=='prep'||!Object.hasOwn(ITEMS,key)||(['massa','molho','queijo'].includes(key)||key===base(a))||!inside(point,.37)||a.toppings.filter(t=>t.kind===key).length>=12||!use(p,key))return false;a.toppings.push({kind:key,x:point.x,y:point.y});return true;}
...
function evaluate(a){const r=recipe(a.recipe),notes=[];const coverage=(a.sauce.length/CELLS.length+a.cheese.length/CELLS.length)/2;let correct=coverage;
 ...
 correct/=(r.toppings.length+1);const extras=new Set(a.toppings.filter(t=>!r.toppings.includes(t.kind)).map(t=>t.kind));correct=Math.max(0,correct-extras.size*.2);if(extras.size)notes.push('Entrou um ingrediente que não estava no pedido.');if(coverage<.8)notes.push('Espalhe molho e o queijo pedido por mais partes da massa.');
...
function settle(p){if(p.active?.stage!=='serve')return null;const result=evaluate(p.active);p.cash+=result.pay+result.tip;p.served++;p.revenue+=result.pay;p.tips+=result.tip;p.best=Math.max(p.best,result.score);p.last=result;p.recent=[...(p.recent||[]),result.recipe].slice(-3);p.clientOrders={...p.clientOrders,[result.client]:result.recipe};p.active=null;return result;}
...
function help(p){if(p.active&&p.active.stage!=='order')return false;const required=p.active?needs(p.active):['massa','molho','queijo'];const lack=required.filter(k=>p.stock[k]<1);if(!lack.length||p.cash>=lack.reduce((sum,k)=>sum+ITEMS[k].cost,0))return false;for(const k of lack)p.stock[k]+=2;return true;}
const API={ITEMS,RECIPES,CELLS,base,brushTool,needs,fresh,normalize,recipe,inside,newOrder,missing,start,brush,topping,moveTo,tick,cut,evaluate,settle,buy,help};if(typeof module!=='undefined')module.exports=API;else root.PistachioPizza=API;
