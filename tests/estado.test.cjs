...
const packed=S.pack(original);assert.equal(S.unpack(packed).state.estrelinhas,35);assert.throws(()=>S.unpack(packed.replace('"estrelinhas":35','"estrelinhas":999')));assert.throws(()=>S.unpack('{}'));assert.throws(()=>S.unpack('[]'));
